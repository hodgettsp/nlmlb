...
```{r rejtnkjn, echo=F, warning=F, message=F}
source(here("script/rmarkdown/plots/nl_mlb_playercount.R"))
```

```{r nsiadnkjn, echo=F, message=F, warning=F, fig.align='center', fig.height=8, fig.width=12, fig.cap="The number of players for a given season in Major League Baseball who also played in the Negro Leagues, 1947 to 1981."}
p
```

```{r tetstuko, echo=F, message=F, warning=F, fig.align='center', fig.height=8, fig.width=12, fig.cap="The number of players for a given season in Major League Baseball who also played in the Negro Leagues, 1947 to 1981."}
# pipe nl_mlb_counts
nl_mlb_counts %>%
...
```{r mlb-counts, echo=F, message=F, warning=F, results='asis'}
# pipe mlb_counts
nl_mlb_counts %>%
  # select the necessary columns
  select(season, player_count, nlp_count, nlp_per_pop, nlp_per_team) %>%
  # keep only distinct values
  distinct() %>%
  # generate table with kbl function from kable Extra
  kableExtra::kbl(caption = 
                    "Negro League Players in MLB - Counts and Proportions: CNLBR and Retrosheet Data.", 
                  booktabs = T, col.names = c("Season", "MLB Population", "NL Player Count", 
                                              "NL Players per MLB Population", "NL Players per Team")) %>%
  # style table, center table, hold its position, set table to full width
  kableExtra::kable_styling(position = "center",
