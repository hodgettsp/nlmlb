...
wiki_counts %>%
  ...
  rename(Season = season,
         `NL Player Debut Count` = debut_count,
         `NL Player Final Game Count` = final_count,
         `NL Player Count` = player_count) %>%
  # remove dummy rows
  slice(-1, -84) %>%
...
nl_mlb_counts %>%
  ...
  distinct() %>%
  # rename columns to something more presentable
  rename(Season = season,
         `MLB Population` = player_count,
         `NL Player Count` = nlp_count,
         `NL Players per MLB Population` = nlp_per_pop,
         `NL Players per Team` = nlp_per_team) %>%
  # generate table with kbl function from kable Extra
  kableExtra::kbl(caption = "Negro League Players in MLB - Counts and Proportions: CNLBR and Retrosheet Data.", booktabs = T) %>%
