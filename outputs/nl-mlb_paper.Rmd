...
# pipe mlb_counts
nl_mlb_counts %>%
  select(played_nl, year, player_count, split_player_count, nlp_per_team, nlp_per_mlb_pop) %>% 
  distinct() %>% 
  filter(played_nl == 1) %>% 
  select(-played_nl) %>%
  # rename the columns, you know the drill
  rename(`MLB Season` = year,
         `MLB Player Count` = player_count,
         `NL Pop in MLB` = split_player_count,
         `NL Players per MLB Population` = nlp_per_mlb_pop,
         `AVG NL Players per Team` = nlp_per_team) %>%
  # generate table using kbl from kableExtra package
  kableExtra::kbl(caption = "Negro League Players in MLB - Counts and Proportions, 1947 - 1980: Retrosheet and CNLBR Data.", 
                  booktabs = T) %>%
  # set table styling
  kableExtra::kable_styling(position = "center", 
