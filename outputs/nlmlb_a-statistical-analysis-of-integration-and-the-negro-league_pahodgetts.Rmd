...
aa <- z %>% 
  ...
  mutate(player_count = abs((debut_count + lag(player_count, default = 0)) - lag(final_count, default = 0)))

for(i in 2:length(aa$player_count)){
  aa$player_count[i] <- (aa$debut_count[i] + aa$player_count[i-1]) - aa$final_count[i-1]
}
```
